...
const TradeCard = ({ trade, onClick }) => {
  ...
  return (
    <div
      ...
      className={`rounded-2xl border border-white/10 bg-[#1b1b1b] p-4 shadow-sm shadow-black/30 ${
        onClick ? "cursor-pointer transition hover:border-gray-700" : ""
      }`}
    >
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-black/25 px-3 py-2.5">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
          {statusDateValue && (
            <span>
              {statusDateLabel} {new Date(statusDateValue).toLocaleDateString("en-IN")}
            </span>
          )}
        </div>
      </div>
